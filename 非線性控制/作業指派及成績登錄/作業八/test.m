...
ax(1) = gca ;
xlabel('Time (sec)') 
ylabel('Output Response','Interpreter','Latex')
xlim([0 t_final])
ylim([0 1.5])
axis normal
grid on
...
ax(5)=gca;
hs(3)=legend({'$r(t)=1$','$r(t)=2sint+cost$'},'Interpreter','latex');
xlabel('Time (sec)') 
ylabel('Output Tracking Error $e(t)$','Interpreter','Latex') 
xlim([0 t_final])
ylim([-0.3 0.3])
...
f14=figure;
plot(t1,(ap-4)/bp*ones(length(t1),1),'--','Color',[0.5 0.5 0.5],'LineWidth',LW); hold on ;
plot(t2,Yt2(3,:),'r','LineWidth',LW) ; 
...
f16 = figure ;
plot(t1,4/bp*ones(length(t1),1),'--','Color',[0.5 0.5 0.5],'LineWidth',LW); hold on ;
plot(t2,Yt2(5,:),'r','LineWidth',LW) ;
plot(t1,Yt1(5,:),'b--','LineWidth',LW) ; 
hs(12)=legend({'$K^*_f$','$r(t) = 2sin(t)+cos(t)$','$r(t) = 1$'},'Interpreter','latex');
ax(16) = gca ;
xlabel('Time (sec)') % x label
ylabel('Estimated Parameter $\hat{K}_r$','Interpreter','Latex') 
xlim([0 t_final])
axis normal
grid on
...
e=y-ym;
u=Ky_est*y+Kf_est*sin(y)+Kr_est*r;
dKy=-sign(bp)*e*y*gamma1;
dKf=-sign(bp)*e*sin(y)*gamma2;
dKr=-sign(bp)*e*r*gamma3;

% Nonlinear System
dy=bp*u-ap*y-cp*sin(y);
